...
const CREDENTIAL_PATH = process.env.HOME
    ? path_1.resolve(path_1.resolve(process.env.HOME, '.config'), CREDENTIAL_SUFFIX)
    : path_1.resolve('.', CREDENTIAL_SUFFIX);
...
function fromPath(filePath) {
    let jsonString;
    try {
        jsonString = fs_1.readFileSync(filePath, 'utf8');
    }
    catch (ignored) {
...
class Credential {
    getToken() {
        return __awaiter(this, void 0, void 0, function* () {
            throw new error_1.CatalystAppError('credential', 'getToken is not implemented');
        });
    }
}
...
class CookieCredential extends Credential {
    ...
    getToken() {
        return __awaiter(this, void 0, void 0, function* () {
            const zcrf_header = this.getZCSRFHeader();
            return Promise.resolve({ cookie: this.cookie, zcrf_header });
        });
    }
}
exports.CookieCredential = CookieCredential;
class CatalystCredential extends Credential {
    constructor(credObj, user = CREDENTIAL_USER.admin) {
        super();
        this.adminType = getAttr(credObj, 'adminType', CREDENTIAL_HEADER.admin_type);
        this.adminToken = getAttr(credObj, 'adminToken', CREDENTIAL_HEADER.admin_token);
        this.userType = credObj[CREDENTIAL_HEADER.user_type];
    ...
    getToken(user = this.currentUser) {
        return __awaiter(this, void 0, void 0, function* () {
            switch (user) {
                ...
                case CREDENTIAL_USER.user:
                    if (this.userCred === undefined) {
                        if (this.currentUser !== this.defaultUser) {
                            return this.getToken(this.defaultUser);
                        }
                        else {
                            throw new error_1.CatalystAuthError('invalid_credential', 'default user is found to be the same as current user', this.defaultUser);
                        }
                    }
        ...
        if (givenUser === undefined) {
            switch (this.currentUser) {
                case CREDENTIAL_USER.admin:
                    ...
                    break;
                case CREDENTIAL_USER.user:
                    this.currentUser = CREDENTIAL_USER.admin;
                    break;
            }
            return;
        }
