...
class CatalystAppInternals {
    constructor(credential) {
        this.credential = helper_1.copyInstance(credential);
    }
    ...
    authenticateRequest(req) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.credential instanceof credential_1.AccessTokenCredential ||
                ...
                if (validator_1.isNonEmptyString(token.access_token)) {
                    this.setOauthHeader(req.headers, token.access_token);
                }
                else if (validator_1.isNonEmptyString(token.ticket)) {
                    this.setTicketHeader(req.headers, token.ticket);
                }
                else if (validator_1.isNonEmptyString(token.cookie)) {
...
class CatalystApp extends CatalystAppInternals {
    constructor(options) {
        try {
            validator_1.isNonNullObject(options, 'options', true);
            validator_1.ObjectHasProperties(options, ['credential'], 'options', true);
            validator_1.isNonNullObject(options.credential, 'options.credential', true);
            validator_1.isValidType(options.credential.getToken, 'function', 'options.credential', true);
            validator_1.isNonEmptyStringOrNumber(options.project_id || options.projectId, 'projectId', true);
        }
        catch (e) {
        ...
        this.config = {
            projectId: parseInt((options.project_id || options.projectId) + ''),
